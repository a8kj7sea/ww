...
/**
 * The StartMechanic class handles the logic for starting a game in the event system.
 * It spawns the event mob, triggers necessary events, and updates the game phase and state.
 */
public class StartMechanic extends GameMechanic {
     ...
     *
     * @param pluginProvider The plugin provider used for configuration and event management.
     */
    ...
    /**
     * Applies the start mechanic to the game, spawning the mob, triggering events, and
     * updating the game state and phase.
     *
    ...
    public void apply(EventGame game) {
        ...
        if (mob == null) {
            throw new IllegalStateException("Mob cannot be null. Please check the mob configuration or restart the server.");
        }
    ...
    /**
     * Checks if the start mechanic can be applied based on the game state and next phase.
     *
     * @param state The current game state.
     * @param phase The next phase of the game.
     * @return true if the game state is IDLE and the next phase is START, otherwise false.
     */
    @Override
